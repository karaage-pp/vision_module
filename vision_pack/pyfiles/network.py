...
#                    session.run(tf.get_variable(subkey).assign(data))
                ...
                for subkey in ('weights', 'biases'):
                    session.run(tf.get_variable(subkey).assign(data_dict[key][subkey]))
...
def fe(input, num_in, num_out, name, relu=True):
    with tf.variable_scope(name) as scope:
        weights = make_var('weights', shape=[num_in, num_out])
        biases = make_var('biases', [num_out])
        features = tf.nn.xw_plus_b(input, weights, biases, name=scope.name)
        if relu:
            fc = tf.nn.relu(features)
        else:
            fc = features
        return fc, features
